...
# ======================================
df = pd.read_csv(r"C:\Users\79036\Downloads\IMBD.csv")
print(f"Загружено {len(df)} фильмов")

# Обрабатываем жанры (разделяем по запятой) и сразу чистим от пробелов
df['genre_list'] = df['genre'].str.split(', ').apply(lambda x: x if isinstance(x, list) else [])
print(df['genre_list'].head())
# Получаем все уникальные жанры
all_genres = []
for genress in df['genre_list']:
...
user_types = [
    {
        'name': 'action_fan',
        'like_genres': ['Action', 'Adventure', 'Sci-Fi'],
        'dislike_genres': ['Drama', 'Romance'],
        'bias': 0.5,
        'noise': 0.5,
        'count': 150
    },
    {
        'name': 'drama_fan',
        'like_genres': ['Drama', 'Romance', 'History'],
...
def calculate_rating(movie_genres, user_profile):
    """
    movie_genres: список жанров фильма
...
print(f"Уникальных фильмов: {ratings_df['movie_name'].nunique()}")
